...
async fn print_widgets_table(entries: &[wezztershier_core::parser::ConfigEntry], show_layout: bool) -> Result<()> {
    if entries.is_empty() {
    ...
    if show_layout {
        ...
        // Show a simplified layout summary since we can't access the internal widgets HashMap
        for widget_id in builder.widget_ids() {
            if let Some(widget) = builder.get_widget(&widget_id) {
...
fn print_layout(container: &wezztershier_core::layout::LayoutContainer, indent: usize) {
    let prefix = "  ".repeat(indent);
    println!("{}📦 {} ({})", prefix, container.title.bold(), format!("{:?}", container.container_type).dimmed());
    
    for widget_id in &container.widgets {
        println!("{}  🔧 {}", prefix, widget_id.cyan());
    }
...
async fn handle_gui(args: GuiArgs) -> Result<()> {
    println!("{}", "🎨 Launching Wezzte GUI...".magenta().bold());
    
    if let Some(file) = args.file {
        println!("Loading configuration: {}", file.display().to_string().cyan());
    }
    
    println!("Starting development server on port {}", args.port.to_string().yellow());
    println!("{}Navigate to: {}", "🌐 ".green(), format!("http://localhost:{}", args.port).blue().underline());
    
    // In a real implementation, this would start the Tauri application
    println!("{}Use 'cargo tauri dev' to start the GUI application", "💡 ".yellow());
